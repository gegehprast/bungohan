...
- State is a **tree**: an instance has one parent at a time. Moving it (remove here, add there) is supported; sharing it between two parents is not. A nested Schema field refuses an instance that is still attached elsewhere (§5.7.9).
...
- **[DECIDED] A nested field only accepts an unattached instance.** Because receivers rebind rather than adopt, taking an instance that is still attached elsewhere would give it a new refId while the receivers' copy stays on the old one, orphaned and silently frozen (e.g. `holder.nested = shared` while `shared` is still in a map: the client's map entry stops updating). So the setter **refuses** an instance that is in a collection, in another nested field (of any instance), or is a room's state (the root, refId 0), and also `owner` itself or one of its ancestors (a cycle). A refused assignment leaves the field on its old value, so server and clients stay in agreement, and logs a `console.error` naming `Class.field`, where the instance is held, and how to move it. It never throws: the setter runs on per-tick paths (CLAUDE.md, rule 1). **To move an instance, remove it first, then assign it**: delete it from its collection, or assign something else to the nested field holding it. Doing both in the same tick is fine: it is sent in full under a new block, as above. This is a server-side rule with no wire change. The check reads `_parent`, so an instance sitting in a collection whose owner isn't initialized yet (not linked, not on the wire) isn't seen as attached.
