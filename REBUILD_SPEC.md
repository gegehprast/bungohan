...
- **Ordering that client-js gets from a 0 ms timer** (§7.5, frames held during a first join) falls out of the pump instead: the pending join is settled first — its continuation resumes inline and registers handlers — and the held frames are released immediately after. The **unclaimed-event** rule is kept for handlers registered later still.
...
**Known gap, found while adding typed options (not fixed here).** The unclaimed-event rule (§7.5) is only half there in C#: a pre-join contract message is kept and delivered to the first `OnMessage` registered for it, but raw messages, `ClientJoined`/`ClientLeft` and `ErrorReceived` are plain C# events. They are kept too, but subscribing with `+=` can't claim them, so a raw message a room sends from `onJoin` never reaches a C# handler attached after the join. client-js delivers it. Fixing it needs custom event accessors that claim, the way `OnMessage` does. The interop `options` room therefore also answers a raw `"options"` request.
