...
abstract class Room<TState extends Schema = Schema, TContract extends Contract = EmptyContract>
  ...
  setSimulationTickRate(fps: number): void;          // [FIX] 0 stops the loop (§6.8.2)
  setStateSyncTickRate(hz: number): void;
  protected syncNow(): void;                         // [NEW] a sync boundary now (§6.8.2)
...
for a broadcast. **[DECIDED]** `gatherTimeout` is an upper bound, not a
fixed cost: a broadcast (`pi?`, `q?`, `find?`, `loc?`) waits only for the
peers the registry believes alive, and ends once each has answered or been
declared dead, at once with none (no message is published). `find?` and
`loc?` used to be answered only by a process that had a match, so a miss
always cost the whole window, and a `reserve` that created its room paid
it twice (≈400 ms, even in a cluster of one); a process without a match
now replies `miss`. That is a new message type, so `CLUSTER_PROTOCOL`
stays 2: an older process ignores `miss`, and never sends one, so it is
waited for until the window passes, as before. During a process's first
`heartbeatInterval` after `start()` its registry may lack a live peer (one
whose `hello` reply hasn't arrived), and ending early would miss that
peer's rooms and create a duplicate; broadcasts then wait the whole window
(`ClusterNode._expected`, `packages/testing/src/cluster/gather.test.ts`). A request waiting on a peer that is declared dead fails at
...
- **Held seats and unconsumed reservations for its rooms are gone with it.**
  ...
  `RESERVATION_NOT_FOUND`, each after at most one collection window.
...
- **`autoDispose`** fires when the last seat *and* the last reservation are gone. A room created by `matchMaker.createRoom` that nobody ever joins lives until disposed, as before. **[DECIDED]** A join that fails before completing (refused by `onAuth`, `onJoin` threw) empties the room only if the room was there for it: the join created it (`joinedBy === "create"`), or it consumed the reservation that kept the room (`"reservation"`). A failed join into an existing room by id, or a bot's `room.join`, leaves the room as it was. Before, any refused join into a `createRoom`-made, still-empty room disposed it, so anyone who learned the room's id could destroy it.
- **[DECIDED] `client.joinedBy`**: `"reservation" | "join" | "create" | "server"`, how the seat was taken, set by the server at each `Client` construction (a forwarded join is built on the owning process by the same paths, so it gets the same value) and before `onAuth`. It's a fact the framework alone knows, and join options can be forged, so it is exposed; what a room does with it is game logic ("only players the lobby placed" is `onAuth` returning `client.joinedBy === "reservation"`), so there is no reservation-only room mode. A reconnection keeps the seat, and with it the value. A `Client` built by server code defaults to `"server"`.
...
- **[DECIDED] The simulation loop is optional.** Its only job is calling `onTick`, so a room that doesn't override `onTick` (checked against `Room.prototype.onTick` when the loops are built) gets no `SimulationLoop` at all: a turn-based room or a lobby, driven by messages and clock timers, costs no idle wake-up. `setSimulationTickRate(0)` removes a ticking room's loop until a positive rate rebuilds it; negative and non-finite rates are still ignored. `RoomMetrics.simulationTicks` stays 0 for such a room.
- **[DECIDED] `protected syncNow()`** runs one sync boundary now (`_syncNow`, the loop's own callback), for rooms whose state changes only in handlers and timers: without it an action reaches clients up to one sync period later (50 ms at the default 20 Hz). It is an ordinary boundary (`onBeforeSync`, backpressure, snapshots for waiting clients, nothing on an idle call) and leaves the loop's schedule alone. It is protected, since it's for the room's own handlers, so `RoomProxy` needs no remote op for it. A call from `onBeforeSync` (or from anything a sync runs) does nothing rather than recurse. A "sync after every message" room option was rejected: the call makes the extra frames visible where they are sent, and a busy room shouldn't get them by default.
