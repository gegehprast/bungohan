...
`h.server` is the real `BungohanServer`. `getRoom(id)` returns the room
instance. `state` is `protected` on `Room`, so declare it `public override
state = …` in your room class to read it from tests, as the tutorial does.
